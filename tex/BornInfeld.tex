...
\begin{document}
  ...
  \begin{frontmatter}
    ...
    \title{Emergent Spacetime Geometry from Bounded Relational Relaxation}

    \author{J\'er\^ome Beau}
    \affiliation{
      organization={Independent Researcher (Cosmochrony Project)},
      city={Paris},
      country={France}
    }
    ...
    \begin{abstract}
      We investigate the structural conditions under which non-linear field dynamics
      and effective spacetime geometry emerge from a relational operator framework
      with irreversible relaxation and finite transport capacity.
      We show that if the projection from relational configurations to effective
      observables is required to support non-trivial gauge structure,
      then it must be non-injective.
      Non-injectivity implies a finite maximal transport capacity for admissible
      relational flux.

      Under locality, Lorentz compatibility, and analyticity,
      finite transport capacity excludes purely quadratic effective actions.
      We prove that the minimal non-linear completion consistent with gauge invariance
      and bounded field invariants is of the Born--Infeld type.

      Starting from a weighted relational Laplacian,
      we derive a continuum limit in which the effective metric tensor arises from
      the principal symbol of the operator,
      while antisymmetric perturbations generate a $U(1)$ gauge field strength.
      ...
      the same mechanism yields the Schwarzschild geometry as the universal exterior solution.

      Horizon formation is shown to correspond to saturation of admissible relational
      flux and to a breakdown of continuum projectability rather than to a curvature singularity.
      These results establish Born--Infeld electrodynamics, Lorentzian geometry,
      and horizon structure as necessary consequences of bounded relational transport
      in a non-injective projection framework.
    \end{abstract}

    \begin{keyword}
      Emergent spacetime geometry \sep Bounded relational relaxation \sep Born--Infeld dynamics \sep
      Relational operators \sep Non-linear field theory \sep Horizons
    \end{keyword}
  ...
  \section{Introduction}
    ...
    elliptic operator whose principal symbol defines an effective metric tensor~\citep{Beau2026a}.
    ...
    representability~\citep{Beau2026b}.
  ...
  in~\citep{Beau2026a}; here we summarize only the elements required to
  ...
  As shown in~\citep{Beau2026a}, this symbol provides a natural,
  ...
  relaxation process requires that the operator remains hyperbolic~\citep{HormanderPDE},
  ...
  Born--Infeld-type field theories~\citep{GibbonsBornInfeldCausality}.
  ...
  effective description rather than a breakdown of the underlying dynamics~\citep{Beau2026b}.
  ...
  Laplacians~\citep{Beau2026a}, we have shown that the bounded-flux constraint does more
  ...
  This interpretation is consistent with structural analyses of non-injective effective
  descriptions and clarifies the operational meaning of strong-field regimes~\citep{Beau2026b}.
